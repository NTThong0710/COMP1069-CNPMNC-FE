...
exports.getAllSongs = async (req, res) => {
  try {
    ...
    // convert obj -> json to create key
    const cacheKey = `songs:list:genere=${genere || ''}:art=${
      artist || ''
    }:alb=${album || ''}:p=${page}:l=${limit}:s=${sort}`;
...
exports.updateSong = async (req, res) => {
  try {
    ...
    if (song) {
      ...
      await clearCacheByPattern('song:*');
      // xoa cache recommendation for this song
      await clearCacheByPattern(`recommendation:song:${req.params.id}`);
    }
...
exports.getRecommendedSongs = async (req, res) => {
  try {
    const { id } = req.params;
    const cacheKey = `recommendation:song:${id}`;

    const cachedRecs = await pubClient.get(cacheKey);
    if (cachedRecs) {
      return res.json(JSON.parse(cachedRecs));
    }

    let song = mongoose.Types.ObjectId.isValid(id)
      ? await Song.findById(id)
      : await Song.findOne({ spotifyId: id });
    if (!song) {
    ...
    const results = { base: song.title, recommendations: relatedSongs };
...
exports.getNewRelease = async (req, res) => {
  const limit = parseInt(req.query.limit) || 10;
  const cacheKey = `song:new-release:limit_${limit}`;

  try {
    // check redis
    const cachedData = await pubClient.get(cacheKey);
    if (cachedData) {
      console.log('Serving new release from cache');
      return res.json(JSON.parse(cachedData));
    }
