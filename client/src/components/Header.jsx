...
const BASE_API_URL = import.meta.env.VITE_API_URL || 'https://comp1069-cnpmnc-be.onrender.com/api';
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            ...
            try {
                ...
                if (!useAI) {
                    const res = await fetch(`${BASE_API_URL}/search/suggest?q=${encodeURIComponent(searchTerm)}&limit=5`);
                    if (res.ok) {
                ...
                const [semanticRes, oldSuggestRes] = await Promise.allSettled([
                    ...
                    fetch(`${BASE_API_URL}/search/suggest?q=${encodeURIComponent(searchTerm)}&limit=3`)
                ]);
                ...
                if (semanticRes.status === 'fulfilled' && semanticRes.value.ok) {
                    ...
                    if (semanticData.success && semanticData.results) {
                        ...
                        const semanticMatches = rawSongs.filter(s => 
                            !s.title.toLowerCase().includes(searchTerm.toLowerCase())
                        );
                        
                        newSuggestions.songs = [...exactMatches, ...semanticMatches].slice(0, 5);
                    }
                }
    ...
    return (
        <>
                    ...
                    <div ref={dropdownRef} className="relative w-full max-w-md group">
                        <form onSubmit={handleSearchSubmit} className="relative flex items-center">
                            ...
                            <input
                                ...
                                placeholder={isUserLoggedIn ? (useAI ? "Ask AI about music..." : "Search song, artist...") : "Log in to search"}
                                className={`w-full rounded-full py-3 pl-10 pr-24 transition-colors shadow-sm focus:outline-none ${isUserLoggedIn ? "bg-[#242424] text-white placeholder-neutral-400 hover:bg-[#2a2a2a] focus:ring-2 focus:ring-white/20" : "bg-[#1a1a1a] text-neutral-500 placeholder-neutral-600 cursor-not-allowed border border-transparent"}`}
                            />

                            {/* GROUP NÚT BÊN PHẢI (CLEAR + AI) */}
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                {searchTerm && isUserLoggedIn && (
                                    <button type="button" onClick={handleClearSearch} className="text-neutral-400 hover:text-white p-1.5 rounded-full hover:bg-neutral-700 transition">
                                        <X size={18} />
                                    </button>
                                )}
                                ...
                                {isUserLoggedIn && (
                                    <button 
                                        ...
                                        className={`p-1.5 rounded-full transition-all flex items-center justify-center ${useAI ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/50 scale-105' : 'bg-neutral-700 text-neutral-400 hover:text-white'}`}
                                ...
                                {suggestions.artists?.length > 0 && (
                                    <div className="mb-2 pb-2 border-b border-neutral-700">
                                        ...
                                        {suggestions.artists.map((artist, idx) => (
                                            <div key={idx} onClick={() => handleSuggestionClick(artist.name)} className="px-2 py-2 hover:bg-neutral-700/50 rounded-md cursor-pointer flex items-center gap-3 transition-colors">
                                                 <div className="w-10 h-10 rounded-full bg-neutral-800 flex items-center justify-center overflow-hidden border border-neutral-700">
                                                     {artist.avatar ? <img src={artist.avatar} className="w-full h-full object-cover"/> : <User size={16} className="text-neutral-500" />}
                                                </div>
                                                <span className="text-white text-sm truncate font-bold">{artist.name}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                    ...
                                    <div className="mb-2">
                                        <div className="flex justify-between items-center px-3 py-2">
                                            <p className="text-xs font-bold text-neutral-400 uppercase">
                                                {suggestions.artists?.length > 0 ? "Songs" : "Best Match"}
                                            </p>
                                            {/* Chỉ hiện badge AI khi đang bật chế độ AI */}
                                            {useAI && <span className="text-[10px] bg-purple-600 text-white px-1.5 py-0.5 rounded flex items-center gap-1"><Sparkles size={8}/> AI Search</span>}
                                        </div>
                                        {suggestions.songs.map((song, idx) => (
                                            ...
                                            >
                                                {song.cover ? (
                                                    <img src={song.cover} alt="" className="w-10 h-10 rounded-md object-cover bg-neutral-800 shadow-sm" />
                                                ) : (
                                ...
                                {suggestions.albums?.length > 0 && (
                                    <div className="mb-2 border-t border-neutral-700 pt-2">
                                        ...
                                        {suggestions.albums.map((album, idx) => (
                                            <div key={idx} onClick={() => handleSuggestionClick(album.title)} className="px-2 py-2 hover:bg-neutral-700/50 rounded-md cursor-pointer flex items-center gap-3 transition-colors">
                                                ...
                                                <span className="text-white text-sm truncate">{album.title}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    {isUserLoggedIn ? (
                        <div className="flex items-center gap-3" ref={userMenuRef}>
                            ...
                            <div className="relative">
                                <button onClick={() => setShowUserMenu(!showUserMenu)} className="bg-purple-800 text-white w-9 h-9 rounded-full flex items-center justify-center font-bold border-2 border-transparent hover:border-white hover:scale-105 transition cursor-pointer overflow-hidden">
                                    {user?.avatar ? <img src={user.avatar} alt="User avatar" className="w-full h-full object-cover" /> : (user?.username ? user.username.charAt(0).toUpperCase() : "U")}
                                </button>
                                {showUserMenu && (
                                    <div className="absolute top-full right-0 mt-2 w-48 bg-[#282828] rounded-md shadow-xl border border-neutral-700 overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-1 z-50">
                                        <Link to="/profile" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-200 hover:bg-[#3E3E3E] rounded-sm transition"><User size={16} /> Profile</Link>
                                        <button onClick={handleLogoutClick} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-neutral-200 hover:bg-[#3E3E3E] rounded-sm transition text-left"><LogOut size={16} /> Log out</button>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {isLogoutModalOpen && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setIsLogoutModalOpen(false)}>
                    <div className="w-full max-w-sm bg-[#282828] rounded-xl shadow-2xl border border-white/5 p-6 text-center transform scale-100 transition-all" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-center mb-4"><div className="w-12 h-12 bg-neutral-700 rounded-full flex items-center justify-center"><LogOut className="text-white w-6 h-6 ml-1" /></div></div>
                        <h3 className="text-xl font-bold text-white mb-2">Đăng xuất?</h3>
                        <div className="flex gap-3 justify-center mt-6">
                            <button onClick={() => setIsLogoutModalOpen(false)} className="px-6 py-2.5 rounded-full font-bold text-white text-sm border border-neutral-600 hover:border-white hover:bg-neutral-800 transition min-w-[100px]">Huỷ</button>
                            <button onClick={confirmLogout} className="px-6 py-2.5 rounded-full font-bold text-black text-sm bg-green-500 hover:bg-green-400 hover:scale-105 transition shadow-lg shadow-green-900/20 min-w-[100px]">Đăng xuất</button>
                        </div>
                    </div>
                </div>,
                document.body
