...
export default function Sidebar({ isLoggedIn }) {
  ...
  const [userLibrary, setUserLibrary] = useState([
    { title: "Liked Songs", subtitle: "Playlist • 127 songs", image: "https://placehold.co/100x100?text=♥" },
  ]);
  ...
  const handleCreatePlaylistClick = (event) => {
    ...
    setUserLibrary([newPlaylist, ...userLibrary]);
  };
        ...
        {isLoggedIn ? (
          ...
          !isCollapsed && (
            <div className='overflow-y-auto px-2 space-y-2'>
                ...
                <div className="bg-neutral-800 rounded-lg p-4 m-2 text-white">
                    <p className="font-bold text-sm mb-1">Let's find some podcasts</p>
                    <p className="text-xs mb-4">We'll keep you updated</p>
                    <button className="bg-white text-black text-xs text-[13px] font-bold px-6 py-2 rounded-full hover:scale-105 transition">Browse podcasts</button>
                </div>
            </div>
          )
        )}
