...
export default function PlaylistPage({ onSongSelect }) {
  ...
  // Listen khi bài hát được thêm vào playlist
  useEffect(() => {
    const handleSongAdded = (event) => {
      // Nếu event là về playlist hiện tại, refetch
      if (event.detail?.playlistId === playlistId) {
        // Refetch dữ liệu playlist
        const fetchUpdatedPlaylist = async () => {
          try {
            ...
            console.error("Lỗi refetch playlist:", err);
          }
        };
    ...
    window.addEventListener("songAddedToPlaylist", handleSongAdded);
    return () => window.removeEventListener("songAddedToPlaylist", handleSongAdded);
  }, [playlistId, BASE_API_URL]);
